...
### Run on the local EPR/RAwR datasets with the same node split setting (85/5/10)
From folder `src`:
```
python train_model.py --dataset Cora --net GCN --dataset_source epr_rawr --epr_root ../.. --data_split rawr --random_sort No
```
Run only JDR on all 9 EPR/RAwR datasets:
```
source run_jdr_rawr.sh
```
Run JDR for `GCN`, `GAT`, `GIN` with no node features and automatically export CSV + LaTeX table:
```
python run_jdr_backbones_no_features.py
```
Run all methods (JDR + rewiring baselines) on all 9 EPR/RAwR datasets:
```
source run_exp_rawr.sh
```
Disable node features (RAwR-style identity features):
```
python train_model.py --dataset Cora --net GCN --dataset_source epr_rawr --epr_root ../.. --data_split rawr --random_sort No --use_epr_features No
```
