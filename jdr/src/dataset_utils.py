...
EPR_RAWR_DATASETS = {
    ...
    'texas': 'Texas',
    'wisconsin': 'Wisconsin',
    # CanYouHearMeNow long-range datasets.
    'caterpillar': 'CanYouHearMeNow/Caterpillar',
    'grid': 'CanYouHearMeNow/Grid',
    'ladder': 'CanYouHearMeNow/Ladder',
    'line': 'CanYouHearMeNow/Line',
    'lobster': 'CanYouHearMeNow/Lobster',
    'tree': 'CanYouHearMeNow/Tree',
    'canyouhearmenow/caterpillar': 'CanYouHearMeNow/Caterpillar',
    'canyouhearmenow/grid': 'CanYouHearMeNow/Grid',
    'canyouhearmenow/ladder': 'CanYouHearMeNow/Ladder',
    'canyouhearmenow/line': 'CanYouHearMeNow/Line',
    'canyouhearmenow/lobster': 'CanYouHearMeNow/Lobster',
    'canyouhearmenow/tree': 'CanYouHearMeNow/Tree',
}
...
class SingleGraphDataset:
    def __init__(self, data):
        self.data = data
        self.__num_classes__ = int(torch.unique(data.y).numel())
        self.__num_features__ = int(data.x.size(1))
    ...
    def __getitem__(self, idx):
        if idx != 0:
            raise IndexError(f"SingleGraphDataset only supports index 0, got {idx}")
        return self.data


def _candidate_epr_roots(epr_root):
    raw = osp.expanduser(str(epr_root))
    cwd = os.getcwd()
    src_dir = osp.dirname(osp.abspath(__file__))
    repo_root = osp.abspath(osp.join(src_dir, '..'))
    ...
    if osp.isabs(raw):
        ...
        candidates = [
            osp.abspath(raw),
            osp.abspath(osp.join(cwd, raw)),
            osp.abspath(osp.join(src_dir, raw)),
            osp.abspath(osp.join(repo_root, raw)),
        ]

    # Helpful fallbacks when JDR and EPR are sibling repos.
    candidates.extend([
        osp.abspath(osp.join(repo_root, 'EPR')),
        osp.abspath(osp.join(repo_root, '..', 'EPR')),
    ])
...
def _load_epr_rawr_dataset(name, normalize_data, epr_root, use_epr_features=True):
    dataset_key = name.lower()
    if dataset_key not in EPR_RAWR_DATASETS:
        supported = ', '.join(sorted(EPR_RAWR_DATASETS.values()))
        raise ValueError(
            f"dataset {name} not supported for dataset_source=epr_rawr. "
            f"Supported datasets: {supported}"
        )

    canonical_name = EPR_RAWR_DATASETS[dataset_key]
    ...
    label_path = None
    tried = []
    for root in _candidate_epr_roots(epr_root):
        curr_base = osp.join(root, canonical_name)
        # canonical_name may contain path separators (e.g., CanYouHearMeNow/Tree).
        file_stem = osp.basename(canonical_name)
        curr_edge = osp.join(curr_base, f"{file_stem}.edgelist")
        curr_feat = osp.join(curr_base, f"{file_stem}.x")
        curr_label = osp.join(curr_base, f"{file_stem}.y")
        if osp.isfile(curr_edge) and osp.isfile(curr_label) and (not use_epr_features or osp.isfile(curr_feat)):
            base_path = curr_base
            edge_path = curr_edge
            feat_path = curr_feat
            label_path = curr_label
            break
        tried.append(curr_base)

    if base_path is None:
        raise FileNotFoundError(
            f"Could not locate EPR/RAwR files for dataset '{canonical_name}'. "
            f"Tried roots: {', '.join(tried)}"
        )

    edges_df = pd.read_csv(edge_path, sep=r"\s+", header=None, comment="#", usecols=[0, 1])
    edge_index = torch.tensor(edges_df.values.T, dtype=torch.long)
    edge_index = torch.cat([edge_index, edge_index.flip(0)], dim=1)

    # Robust parser: supports both comma-separated and whitespace-separated files.
    y = torch.tensor(
        pd.read_csv(label_path, sep=r"[\s,]+", engine="python", header=None).values.squeeze(),
        dtype=torch.long,
    )
    # Some synthetic/long-range datasets may not use contiguous class ids.
    uniq_labels = torch.unique(y, sorted=True)
    if uniq_labels.numel() > 0:
        remap = {int(v.item()): i for i, v in enumerate(uniq_labels)}
        y = torch.tensor([remap[int(v.item())] for v in y], dtype=torch.long)
    if use_epr_features:
        x = torch.tensor(
            pd.read_csv(feat_path, sep=r"[\s,]+", engine="python", header=None).values,
            dtype=torch.float,
        )
    else:
        # RAwR behavior when no feature file is provided.
        x = torch.eye(y.size(0), dtype=torch.float)

    if x.size(0) != y.size(0):
        raise ValueError(
            f"Node feature/label size mismatch for {canonical_name}: "
            f"x has {x.size(0)} rows, y has {y.size(0)} rows"
        )

    if edge_index.numel() > 0:
        max_node_id = int(edge_index.max().item())
        if max_node_id >= x.size(0):
            raise ValueError(
                f"Edge index out of range for {canonical_name}: "
                f"max node id is {max_node_id}, but x has {x.size(0)} rows"
            )
...
def DataLoader(name, normalize_data, dataset_source='pyg', epr_root='../..', use_epr_features=True):
    if dataset_source.lower() == 'epr_rawr':
        return _load_epr_rawr_dataset(name, normalize_data, epr_root, use_epr_features)
    if dataset_source.lower() != 'pyg':
        raise ValueError(f"dataset_source '{dataset_source}' not supported")
