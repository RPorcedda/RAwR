...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run JDR experiments for multiple backbones with no node features (RAwR-style identity features)."
    )
    ...
    parser.add_argument("--rpmax", type=int, default=100, help="Number of repeated runs (RPMAX).")
    parser.add_argument("--denoise", type=str, default="Yes", help="Whether to denoise the features.")
    ...
    parser.add_argument("--output_dir", type=str, default="results/rawr_no_features", help="Output directory.")
...
def write_latex(rows: List[Dict[str, object]], models: List[str], datasets: List[str], path: pathlib.Path) -> None:
    ...
    with path.open("w") as f:
        ...
        for model in models:
            ...
            f.write(f"{model} & No & JDR & " + " & ".join(cells) + " \\\\\n")
        f.write("\\hline\n")
        f.write("\\end{tabular}\n")
        f.write("\\caption{Test mean accuracies (\\(\\pm\\) std) for JDR with no node features across backbones and datasets.}\n")
        f.write("\\label{tab:jdr_rawr_no_features_backbones}\n")
        f.write("\\end{table}\n")
...
def main() -> int:
    ...
    csv_path = output_dir / "jdr_no_features_backbones_results.csv"
    tex_path = output_dir / "jdr_no_features_backbones_table.tex"
