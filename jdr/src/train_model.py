...
def RunExp(exp_i, args, dataset, data, Net, percls_trn, val_lb):
    ...
    if args.data_split == 'rawr':
        data = random_rawr_splits(
            data,
            val_ratio=args.val_rate,
            test_ratio=1.0 - args.train_rate - args.val_rate,
            seed=exp_i + args.run_num,
            use_seed=args.rawr_split_seeded,
        )
    elif args.dataset.lower() in ['twitch-gamers']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net', type=str,
                        choices=['GCN', 'GAT', 'GIN', 'APPNP', 'ChebNet', 'JKNet', 'GPRGNN', 'MLP', 'SpectralClustering', 'none'],
    ...
    parser.add_argument('--data_split', default='sparse', choices=['sparse', 'dense', 'gdl', 'sparse5', 'half', 'rawr'],
                        help="sparse: 0.025/0.025, dense: 0.6/0.2, rawr: 0.85/0.05 train/val and 0.10 test")
    ...
    parser.add_argument('--dataset_source', type=str, default='pyg', choices=['pyg', 'epr_rawr'],
                        help="Dataset backend: 'pyg' (default) or local EPR/RAwR files")
    parser.add_argument('--epr_root', type=str, default='../..',
                        help="Path to local reproducibility package root (used when --dataset_source epr_rawr)")
    parser.add_argument('--use_epr_features', type=str, choices=['Yes', 'No'], default='Yes',
                        help="Only for dataset_source=epr_rawr: use .x node features (Yes) or disable them (No)")
    parser.add_argument('--rawr_split_seeded', default=False, action=argparse.BooleanOptionalAction,
                        help="Seed node permutation for rawr split. Disable to mimic RAwR default behavior")
    ...
    parser.add_argument('--RPMAX', type=int, default=5, help="Number of experiments to run (different seeds)")
    ...
    parser.add_argument('--rewire', type=str, default='none',
                        choices=['none', 'ppr', 'heat', 'fosr', 'sdrf', 'borf', 'borf4', 'borf5', 'knn', 'knn_weighted', 'heat_eps', 'ppr_eps'],
    ...
    parser.add_argument('--sdrf_loops', type=int, default=10, help="Number of SDRF rewiring iterations")
    parser.add_argument('--sdrf_tau', type=float, default=1.0, help="Softmax temperature for SDRF edge additions")
    parser.add_argument('--sdrf_removal_bound', type=float, default=0.5, help="Curvature threshold for SDRF edge removals")
    parser.add_argument('--sdrf_curvature', type=str, choices=['bfc', 'orc'], default='bfc', help="Curvature type used by SDRF")
    ...
    if args.data_split == "sparse":
        ...
        args.train_rate = 0.5
        args.val_rate = 0.25
    elif args.data_split == "rawr":
        args.train_rate = 0.85
        args.val_rate = 0.05
    ...
    args.use_epr_features = True if args.use_epr_features == "Yes" else False
    if args.data_split == "rawr" and args.random_sort:
        print("[INFO] Disabling random node sorting to match EPR/RAwR evaluation setting")
        args.random_sort = False
    ...
    dataset, data = DataLoader(
        ...
        epr_root=args.epr_root,
        use_epr_features=args.use_epr_features,
    )
    if args.wandb_log:
    ...
    if args.data_split == "rawr":
        ...
        TrueLBrate = train_rate + val_rate
    else:
    ...
    if args.denoise:
        ...
        if args.denoise_type == 'jointly':
            if args.rewired_index_A > len(data.y):
                ...
                print(
                    "rewired_index is too large, set to the maximum values "
                    f"A={args.rewired_index_A}, X={args.rewired_index_X}"
                )
    ...
    elif args.rewire == 'sdrf':
        adj_pre = to_dense_adj(data.edge_index)[0]
        from preprocessing.sdrf import sdrf as sdrf_rewire
        edge_index, edge_type = sdrf_rewire(
            dataset.data,
            loops=args.sdrf_loops,
            remove_edges=True,
            removal_bound=args.sdrf_removal_bound,
            tau=args.sdrf_tau,
            is_undirected=False,
            curvature=args.sdrf_curvature,
        )
        dataset.data.edge_index = edge_index
        dataset.data.edge_type = edge_type
        data.edge_index = dataset.data.edge_index
        data.edge_type = dataset.data.edge_type
        if args.dataset.lower() not in ['questions', 'twitch-gamers', 'penn94']:
            adj_sdrf = to_dense_adj(data.edge_index)[0]
            if args.wandb_log:
                wandb.run.summary["edges/original"] = (adj_pre == 1).sum().item()
